...
def generate_launch_description():
    ...
    x_arg = DeclareLaunchArgument(
        'intruder_x', default_value=str(defaults['x']),
        description='Intruder X position (m)')
    y_arg = DeclareLaunchArgument(
        'intruder_y', default_value=str(defaults['y']),
        description='Intruder Y position (m)')
    z_arg = DeclareLaunchArgument(
        'intruder_z', default_value=str(defaults['z']),
    ...
    spawn_intruder = Node(
        ...
        arguments=[
            '-name', 'intruder',
            '-x', LaunchConfiguration('intruder_x'),
            '-y', LaunchConfiguration('intruder_y'),
            '-z', LaunchConfiguration('intruder_z'),
            '-string', (
                '<sdf version="1.9">'
                '<model name="intruder">'
                '<static>false</static>'
